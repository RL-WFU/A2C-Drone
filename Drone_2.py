...
class PolicyEstimator_RNN:
    def __init__(self, sight_dim=2, batch_size = 1, train_length = 6, lr=0.001, scope='RNN_model_policy'):
        self.batch_size = batch_size
        self.train_length = train_length
        self.sight_dim = sight_dim
        with tf.variable_scope(scope):
            self.state = tf.placeholder(shape=[None, self.train_length, self.sight_dim * self.sight_dim * 3], dtype=tf.float32, name='state')
...
class ValueEstimator_RNN:
    def __init__(self, sight_dim = 2, batch_size = 1, train_length = 6, lr=0.001, scope='RNN_model_value'):
        self.batch_size = batch_size
        self.train_length = train_length
        self.sight_dim = sight_dim
        with tf.variable_scope(scope):
            self.state = tf.placeholder(shape=[None, self.train_length, self.sight_dim * self.sight_dim * 3],
...
def get_last_t_states(t, episode):
    ...
    states = np.reshape(states, [1, t, sight_dims * sight_dims * 3])
    return states
    ...
    for i_episode in range(num_episodes):
        ...
        for t in range(num_steps):
            ...
            if t < seq_length:
                ...
                states = get_last_t_states(seq_length, episode)
            ...
            if t >= seq_length:
                ...
                states1 = get_last_t_minus_one_states(seq_length, episode)
...
environment = Env()
num_steps = 1000
policy_estimator = PolicyEstimator_RNN()
value_estimator = ValueEstimator_RNN()
sight_dims = 2

with tf.Session() as sess:
    init_op = tf.global_variables_initializer()
    sess.run(init_op)
    actor_critic(environment, policy_estimator, value_estimator, 2000)
