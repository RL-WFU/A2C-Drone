...
class Env:
    ...
    def step(self, action, explore):
        ...
        if not explore:
            percent_covered, _, _, _ = self.miningCovered()

            if percent_covered > .4:
                self.done = True

        else:
            percent_covered = self.totalCovered()
            if percent_covered > .4:
                self.done = True

        return classifiedImage, pursuit_reward, explore_reward, self.done, succ_visits, hover, percent_covered
    ...
    def get_reward(self, classifiedImage, action, explore):
        ...
        pursuit_reward = self.TIMESTEP_PENALTY
        explore_reward = self.TIMESTEP_PENALTY
    ...
    def getVisitedBinaries(self):
        ...
        visiteds = np.ones((self.args.sight_dim * 2 + 1) * (self.args.sight_dim * 2 + 1))
        index = 0
        for i in range(self.args.sight_dim * 2 + 1):
            for j in range(self.args.sight_dim * 2 + 1):
                row_pos = self.row_position - self.args.sight_dim
                col_pos = self.col_position - self.args.sight_dim
                visiteds[index] = self.visited[row_pos, col_pos]
                index += 1
    ...
    def miningCovered(self):
        ...
        return percent_covered, total_mining, total_mining_covered, mining_positions
    ...
    def waterCovered(self):
        ...
        return percent_covered, water_positions
    ...
    def forestCovered(self):
        ...
        return percent_covered, forest_positions
