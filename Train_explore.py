...
def actor_critic(envs, pursuit_policy, pursuit_value, explore_policy, explore_value, num_episodes, discount_factor=0.99):
    ...
    episode_explore_rewards = np.zeros(0)
    episode_lengths = np.zeros(0)
    most_common_actions = []


    interval_explore_averages = np.zeros(args.num_episodes // args.plot_interval)
    interval_explore_totals = np.zeros(args.num_episodes // args.plot_interval)
    ...
    for i_episode in range(num_episodes):
        ...
        episode = []
        episode_explore_rewards = np.append(episode_explore_rewards, 0)
        ...
        for t in range(args.num_steps):
            ...
            if t < args.seq_length:
                ...
                action_probs = explore_policy.predict(state=states, successor_visits=successor_visits, actions=last_actions)
            ...
            next_image, pursuit_reward, explore_reward, done, successor_visit, hover = episode_env.step(action, explore)
            ...
            # Update statistics
            episode_explore_rewards[i_episode] += explore_reward
            ...
            if t >= args.seq_length:
                ...
                explore_value_next = explore_value.predict(states1, successor_visits1, last_actions1)

                td_target_explore = explore_reward + args.gamma * explore_value_next

                explore_td_error = td_target_explore - explore_value.predict(states, successor_visits, last_actions)
                explore_value.update(states, successor_visits, td_target_explore, last_actions, i_episode)
                explore_policy.update(states, explore_td_error, action, successor_visits, last_actions, i_episode)
        ...
        interval_explore_totals[i_episode // args.plot_interval] += episode_explore_rewards[i_episode]
        interval_explore_averages[i_episode // args.plot_interval] = interval_explore_totals[
        ...
        if (i_episode + 1) % args.plot_interval == 0 and i_episode > 0:
            ...
            plt.plot(intervals, interval_explore_averages[0:(i_episode//args.plot_interval + 1)])
        ...
        print("Episode {} finished. Explore Reward {}. Steps: {}. Most Common Action: {}. Hovers: {}.".format(i_episode,
                                                                                          episode_explore_rewards[i_episode],
    ...
    plt.plot(episode_explore_rewards)
    plt.ylabel('Episode reward')
    plt.xlabel('Episode')
...
def test(env, estimator_policy, estimator_value, num_episodes=200, sess=None):
    ...
    for i_episode in range(num_episodes):
        ...
        data = collections.Counter(actions)
        most_common_actions.append(data.most_common(1))
        print("Episode {} finished. Reward: {}. Steps: {}. Most Common Action: {}. Hovers {}.".format(i_episode,
...
parser.add_argument('--seq_length', default=8, type=int, help='number of stacked images')
...
parser.add_argument('--save_dir', default='A2RC_saves/Test52', type=str, help='Save directory')
...
parser.add_argument('--weight_dir', default='A2RC_saves/Test52/Weights', type=str, help='Dir to save/get weights to/from')
...
#Papers
#look up exploration papers, rescue mission papers